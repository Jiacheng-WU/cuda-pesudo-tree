...
__global__ void naive_star_kernel(const data_t* A, const data_t* B,
    ...
    if (thread_idx == 0) {
        static_assert(sizeof(data_t) == sizeof(unsigned long long int));
...
data_t st_naive(const int32_t N, const data_t seed) {
    ...
    Timer::RepeatTiming<Timer::GPU>("ST GPU Naive", timed_func, reset_func);
...
data_t st_pwarp(const int32_t N, const data_t seed) {
    assert(N % NUM_THREADS_IN_BLOCK == 0);
    ...
    auto reset_func = [&]() -> void { d_output[0] = 0; };
    Timer::RepeatTiming<Timer::GPU>("ST GPU PWarp", timed_func, reset_func);
    thrust::host_vector<data_t> h_output = d_output;
    return h_output[0];
}
...
data_t st_torch(const int32_t N, const data_t seed) {
    ...
    Timer::RepeatTiming<Timer::GPU>("ST GPU Torch", compute_func,

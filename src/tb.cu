...
__device__ int64_t sum_over_2nd_dim_warp(const int64_t* Mat, const int32_t N,
                                         int32_t row_idx) {
    ...
    using WarpReduce = cub::WarpReduce<int64_t>;
...
__global__ void pwarp_star_kernel(const int64_t* A, const int64_t* B,
                                  const int64_t* C, const int32_t N,
                                  int64_t* g_output) {
    ...
    for (int32_t row_idx = global_warp_idx * num_rows_local;
        ...
        // Perhaps we could store the intermediate results in shared memory
        // and do the reduction at the end for each row at same time
        int64_t A_i_sum = sum_over_2nd_dim_warp(A, N, row_idx);
        int64_t B_i_sum = sum_over_2nd_dim_warp(B, N, row_idx);
        int64_t C_i_sum = sum_over_2nd_dim_warp(C, N, row_idx);
...
int64_t tb_pwarp(const int32_t N, const int64_t seed) {
    ...
    auto reset_func = [&]() -> void { d_output[0] = 0; };
    Timer::RepeatTiming<Timer::GPU>("GPU PWarp", timed_func, reset_func);
    thrust::host_vector<int64_t> h_output = d_output;
    return h_output[0];
}
